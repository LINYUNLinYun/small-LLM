...
因为全部的pretrain数据集有32GB，总计2900W行text，所以只采用300W用来训练。
...
### 数据清洗
`./dataset/deal_dataset.py`脚本负责对下载的数据集进行清洗和预处理。它会读取原始数据文件，提取文本内容并分块为512字符一块的，方便训练。并将其保存为新的jsonl文件，以便后续的训练使用。

预训练的数据集是30GB左右；sft是10GB左右。

### 训练BPE分词器（待做：用30GB的数据集训练一次分词器）
...
根据mask来计算loss，最后取平均。输入X包含[:n-1]的token，Y包含[1:n]的token(答案)，模型需要根据X中的第[1,i]个token来预测y的第i+1个token，其中i=1,2,...,n-1。对于预训练来说，mask是全1的，所以每个token都要计算loss，如transformer中的上三角矩阵

sft则不同，因为我们不想要模型预测用户的提问，所以只对助手的回答部分计算loss。mask则也有不同。
...
### 预训练 pretrain
首先是要安装下swanlab，然后配置自己的账号。

小样本测试下预训练：
`python pretrain.py --gpus 0 --data_path ./dataset/input/seq_monkey_dealed.jsonl --batch_size 16 --accumulation_steps 8 --learning_rate 2e-4 --epochs 1 --num_workers 4 --max_samples 200000 --out_dir ./checkpoints_1gpu --use_swanlab`
历史半个小时，观察到loss在不断下降，说明预训练的流程是正确的。

![alt text](./pictures/testpretrainloss.png)

#### 正式训练第一次 2000000个样本
`python pretrain.py --gpus 0 --data_path ./dataset/input/seq_monkey_dealed.jsonl --batch_size 24 --accumulation_steps 8 --learning_rate 2e-4 --epochs 1 --num_workers 4 --max_samples 2000000 --out_dir ./checkpoints_1gpu --use_swanlab`

### SFT训练

#### 6000000个样本sft 预计用时11h
`python3 sft.py --gpus 0 --data_path ./dataset/input/BelleGroup_sft.jsonl --epochs 1 --batch_size 32 --accumulation_steps 8 --num_workers 2 --out_dir ./sft_model_215M --max_sample 6000000 --use_swanlab`

### 结果与测试

预训练的损失曲线：
![alt text](./pictures/pretrainloss-final.png)

sft的损失曲线：
![alt text](./pictures/sftloss-final.png)

因为时间有限，所以从分词器到预训练到sft都只采用了一部分的数据集去做，总共的训练时间大概在单卡4090 17h左右，而且显存根本没有跑满因为batchsize只开到16或24或32（32的话大致占用20G显存）。毕竟是215M的小小小模型，也正常。

用model_sample.py测试结果如下：
![alt text](./pictures/result.png)

有一点智能，但是不多。事已至此，虽然比较智障，但是预期的目标还是达到了，跑通了数据采集、数据清洗、训练分词器、预训练、sft的全流程，对LLM的大致流程以及基于Transformer的LLaMA2的架构有了基础的了解。

后续优化：
| todo | 备注 |完成 |
| --- | --- |   --- |
| 加长训练时间 | 如果把数据集全训练的话（大约在百小时这个数量级），也许会好很多 | 未完成 |
| RAG | 知识检索增强 | 未完成 |
| 更大的参数 | 模型架构改进 | 未完成 |
| ui | 让它能和人对话 | 未完成 |

### 总结
2026-03-26 v1.0 本项目已达成历史使命，后续可能不更新了。我会开个新的项目做的更好。

### 参考资料
[1] https://datawhalechina.github.io/happy-llm/#/./README

<!-- [sft](https://swanlab.cn/@linyun/small-LLM/chart/default/8i3b4e7wl37iunbgq86kp/PEz_Hxq7) -->
