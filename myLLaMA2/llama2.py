...
from utils import precompute_freqs
from transformers.modeling_outputs import CausalLMOutputWithPast
from typing import Optional
import math

# 继承了预训练模型类，方便后续加载预训练权重和使用transformers库的功能
class LLaMA2(PreTrainedModel):
    config_class = ModelConfig  # 配置类
    # optional是指这个属性可以是None，或者是torch.Tensor类型。这个属性用来记录最后一次计算的损失值
    ...
    def __init__(self, args: ModelConfig = None):
        ...
        # 将词嵌入层的权重与输出层的权重共享，这里实际上共享的是指针，用的时候计算转置
        self.tok_embeddings.weight = self.output.weight 

        # 预计算相对位置嵌入的频率, 维度是头的
        freqs_cos, freqs_sin = precompute_freqs(self.args.dim // self.args.n_heads, self.args.max_seq_len)
        # 将频率注册为模型的缓冲区，这样它们就会随着模型一起保存和加载，但不会被优化器更新；
        # persistent=False表示这些缓冲区不会被保存到模型的状态字典中，可以节省存储空间，因为这些频率是可以通过预计算函数重新生成的，不需要保存。
    ...
    def generate(self, idx, stop_id=None, max_new_tokens=256, temperature=1.0, top_k=None):
        ...
        在 model.eval() 模式下运行。效率较低的采样版本 没有使用kv cache。
        """
        index = idx.shape[1]
...
if __name__ == "__main__":
    ...
    # 创建一个随机的输入张量，形状为[1, 50]，值在0到6144之间，表示一个批次中有一个序列，序列长度为50，每个元素是一个词汇表中的索引
