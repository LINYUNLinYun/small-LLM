...
from typing import Optional        # 类型提示，表示参数可以是某种类型或 None
...
def preprocess(sources, tokenizer, max_len, system_message: str = "You are a helpful assistant."):
    """
    预处理多轮对话数据，将其转换为模型可接受的格式
    
    参数说明：
    - sources: 原始对话数据列表，每个元素是一个对话列表
    - tokenizer: 分词器，用于将文本转换为 token ID
    - max_len: 最大序列长度
